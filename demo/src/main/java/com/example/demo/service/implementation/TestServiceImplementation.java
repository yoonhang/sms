...
public class TestServiceImplementation implements TestService {
    ...
    public ResponseEntity<String> sendSms(String to) {
        ...
        try {
            ...
            if (!result)
                return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("메세지 전송 오류");
        } catch (Exception exception) {
            exception.printStackTrace();
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("메세지 전송 예외 발생");
        }
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("메세지 전송 예외 발생");
